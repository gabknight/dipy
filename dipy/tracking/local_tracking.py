...
class LocalTracking:
    ...
    def __iter__(self):
        ...
        return utils.transform_tracking_output(
            track, self.affine, save_seeds=self.save_seeds
        )
    ...
    def _generate_tractogram(self):
        ...
        for i, s in enumerate(self.seeds):
            ...
            if self.initial_directions is None:
                ...
                directions = (
                    self.initial_directions[i, d_ns > 0, :] / d_ns[d_ns > 0, np.newaxis]
                )
            ...
            for first_step in directions:
                ...
                if not (
                    self.return_all
                    or stream_status
                    in (StreamlineStatus.ENDPOINT, StreamlineStatus.OUTSIDEIMAGE)
                ):
                    continue
...
class ParticleFilteringTracking(LocalTracking):
    ...
    def __init__(
        ...
        *,
        ...
        self.pft_max_nbr_back_steps = int(np.ceil(pft_back_tracking_dist / step_size))
        self.pft_max_nbr_front_steps = int(np.ceil(pft_front_tracking_dist / step_size))
        pft_max_steps = self.pft_max_nbr_back_steps + self.pft_max_nbr_front_steps

        if (
            self.pft_max_nbr_front_steps < 0
